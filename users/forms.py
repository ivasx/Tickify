...
class LoginUserForm(AuthenticationForm):
    username = forms.CharField(label="Логін або E-mail",
                               widget=forms.TextInput(attrs={'class': 'form-input'}), max_length=150)
...
class RegisterUserForm(UserCreationForm):
    ...
    def clean_email(self):
        ...
        if get_user_model().objects.filter(email=email).exists():
            raise forms.ValidationError('Користувач з таким E-mail уже існує.')
        return email
...
class ProfileUserForm(forms.ModelForm):
    ...
    date_birth = forms.DateField(label="Дата народження",
